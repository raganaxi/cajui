...
function BarcodeInputDemo(props: Partial<React.ComponentProps<typeof BarcodeInput>>) {
    ...
    <div className="flex flex-col gap-2 w-80">
      ...
      {scannedCode && (
        <p className="text-sm text-green-500">
          ✓ Escaneado: <strong className="font-mono text-white">{scannedCode}</strong>
        </p>
      )}
    </div>
  )
...
export const InteractiveScan: Story = {
  ...
  play: async ({ canvas, userEvent }) => {
    ...
    await expect(await canvas.findByText(/✓ Escaneado: XYZ-789/i)).toBeVisible();
  },
}
