...
export const Sizes: Story = {
  render: () => (
    <div className="flex flex-col gap-8 items-start p-6 bg-slate-950 rounded-xl text-white">
      ...
      <div>
        <span className="text-xs text-slate-500 block mb-2">Mediano (md - Default)</span>
...
export const ThemeOverrides: Story = {
  render: () => (
    <div className="flex flex-col gap-6 p-6 bg-slate-950 rounded-xl text-white">
      <div>
        <span className="text-xs text-slate-500 block mb-2">Violeta (#8b5cf6)</span>
        <Branding companyName="Violet POS" themeColor="#8b5cf6" glassTint />
      </div>
      <div>
        <span className="text-xs text-slate-500 block mb-2">Rosa (#ec4899)</span>
        <Branding companyName="Pink Retail" themeColor="#ec4899" glassTint />
      </div>
      <div>
        <span className="text-xs text-slate-500 block mb-2">Naranja (#f97316)</span>
        <Branding companyName="Orange Food" themeColor="#f97316" glassTint />
      </div>
      <div>
        <span className="text-xs text-slate-500 block mb-2">Cian (#06b6d4)</span>
...
export const CustomLogo: Story = {
  args: {
    ...
    logoUrl: 'https://www.gstatic.com/images/branding/product/1x/google_cloud_64dp.png',
    glassTint: true,
  },
};
...
export const CssCheck: Story = {
  ...
  play: async ({ canvas }) => {
    ...
    if (container) {
      ...
      expect(computedStyle.getPropertyValue('--caj-primary').trim()).toBe('139 92 246');
    }
  },
};
