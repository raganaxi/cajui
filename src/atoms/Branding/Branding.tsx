...
import type { BrandingProps } from "./interface";
...
export function Branding({
	...
	const renderFallbackLogo = () => (
		<svg
			className={cn(
				"text-caj-primary animate-pulse filter drop-shadow-[0_0_12px_rgba(var(--caj-primary),0.6)]",
				sizeClasses.fallbackLogo,
			)}
			viewBox="0 0 100 100"
			fill="none"
		...
		<div className={cn("flex items-center", sizeClasses.container)}>
			{logoUrl ? (
				<img
					...
					className={cn(
						"object-contain filter drop-shadow-[0_4px_8px_rgba(0,0,0,0.15)]",
						sizeClasses.logo,
					)}
				/>
			) : (
			...
			<span className={cn("text-white", sizeClasses.text)}>{companyName}</span>
		</div>
	);
	...
	if (glassTint) {
		return (
			<GlassPanel
				...
				className={cn(
					"inline-flex items-center justify-center border-caj-primary/20 backdrop-blur-md",
					className,
				)}
			>
				{innerContent}
