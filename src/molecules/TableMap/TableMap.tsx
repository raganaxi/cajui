...
import { TableCard, type TableData, type TableStatus, type TableShape, type TableCardProps } from '@/atoms/TableCard'

export { TableCard }
export type { TableData, TableStatus, TableShape, TableCardProps }
...
export function TableMap({
  ...
  const allSections: TableMapSection[] = sections ?? (tables ? [{ name: '', tables }] : [])
            ...
            {section.tables.map((table) => (
              <div
                ...
                className={cn(
                  ...
                  selectedId === table.id && 'ring-2 ring-caj-primary ring-offset-2',
                )}
              >
                <TableCard
                  table={table}
        ...
        <div className="flex flex-wrap gap-3 border-t border-white/[0.10] pt-3">
          {(Object.entries(STATUS_LABELS) as [TableStatus, string][]).map(([status, label]) => (
            <span key={status} className="flex items-center gap-1.5 text-xs text-white/60">
              <span className={cn('h-3 w-3 rounded-sm border', {
                'border-green-500/50 bg-green-500/20': status === 'available',
                'border-red-500/50 bg-red-500/20': status === 'occupied',
                'border-amber-500/50 bg-amber-500/20': status === 'reserved',
