...
export function SplitPayment({
  ...
  const isValid = Math.abs(totalAssigned - total) < 0.01 && splits.every((s) => s.method !== null && s.amount > 0)
  ...
  function distributeEvenly() {
    ...
    setSplits((prev) =>
      prev.map((s, i) => ({ ...s, amount: i === prev.length - 1 ? remainder : per })),
    )
  }

  function assignRemaining(id: string) {
    const others = splits.filter((s) => s.id !== id).reduce((a, s) => a + s.amount, 0)
    const rem = +(total - others).toFixed(2)
    setSplits((prev) => prev.map((s) => (s.id === id ? { ...s, amount: Math.max(0, rem) } : s)))
  }
  ...
  return (
    <div className={cn('flex flex-col gap-4', className)}>
      ...
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={distributeEvenly} className="text-xs font-semibold text-caj-primary underline underline-offset-2">
          Dividir igual
        </button>
      </div>
      ...
      <div className="flex justify-between text-xs text-white/60">
        <span>Asignado: <strong className="text-white">{fmt(totalAssigned)}</strong></span>
        <span className={remaining > 0.01 ? 'text-amber-300' : 'text-green-300'}>
      ...
      <div className="flex flex-col gap-3">
        {splits.map((split, idx) => (
          <div key={split.id} className="rounded-xl border border-white/[0.18] bg-white/[0.08] backdrop-blur-md p-3">
