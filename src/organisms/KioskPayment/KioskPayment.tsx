...
import type { KioskPaymentProps, KioskPaymentStatus, KioskPaymentOption } from './interface'
...
const DEFAULT_OPTIONS: KioskPaymentOption[] = [
  ...
  },
  {
    ...
    icon: (
      <svg className="h-10 w-10 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.2} d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.907c.961 0 1.36 1.246.582 1.81l-3.97 2.883a1 1 0 00-.364 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.971-2.883a1 1 0 00-1.175 0l-3.97 2.883c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.364-1.118l-3.97-2.883c-.779-.564-.38-1.81.582-1.81h4.907a1 1 0 00.95-.69l1.519-4.674z" />
      </svg>
    ),
  },
]
...
export function KioskPayment({
  ...
  return (
    <GlassPanel
      ...
      strength="medium"
      radius="xl"
      className={cn('flex flex-col overflow-hidden max-w-2xl mx-auto w-full p-6 text-white min-h-[460px] justify-between', className)}
    >
      {/* ── HEADER: Total Amount ── */}
      <div className="flex flex-col items-center text-center gap-1 border-b border-white/10 pb-4">
        <span className="text-sm font-semibold uppercase tracking-wider text-white/50">Monto Total a Pagar</span>
      ...
      <div className="flex-1 flex flex-col items-center justify-center my-6">
        ...
        {status === 'select' && (
          <div className="w-full flex flex-col gap-3">
            <p className="text-center text-sm text-white/60 mb-2 font-medium">Seleccione su forma de pago:</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {finalOptions.map((opt) => (
                ...
                >
                  ...
                  <div>
                    <h3 className="text-base font-bold text-white tracking-wide">{opt.label}</h3>
                    {opt.description && (
                      <p className="text-xs text-white/50 mt-1 leading-normal">{opt.description}</p>
                    )}
                  </div>
                </button>
              ))}
        ...
        {status === 'prompt' && (
          <div className="flex flex-col items-center text-center gap-6 animate-fade-in max-w-sm">
            ...
            <div className="relative flex items-center justify-center h-28 w-28 bg-white/[0.03] border border-white/10 rounded-full [box-shadow:0_0_40px_rgba(255,255,255,0.05)]">
              {/* Radial signal waves */}
              <div className="absolute inset-0 rounded-full border border-caj-primary/40 animate-ping opacity-60" style={{ animationDuration: '2s' }} />
              <div className="absolute inset-2 rounded-full border border-white/20 animate-ping opacity-45" style={{ animationDuration: '3s' }} />
              
              <svg className="h-14 w-14 text-caj-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
              </svg>
            </div>
            ...
            <div className="space-y-2">
              <h2 className="text-2xl font-black text-white tracking-wide">
                {selectedMethod === 'nfc' ? 'Acerque su celular' : 'Inserte o deslice su tarjeta'}
              </h2>
              <p className="text-sm text-white/60 leading-relaxed font-medium">
                Por favor, siga las instrucciones en la terminal de pago que se encuentra debajo de la pantalla.
              </p>
            </div>
          </div>
        )}
        ...
        {status === 'processing' && (
          <div className="flex flex-col items-center text-center gap-6 animate-fade-in">
            ...
            <div className="space-y-1.5">
              <h2 className="text-xl font-bold text-white">Autorizando transacción...</h2>
              <p className="text-sm text-white/50 animate-pulse font-medium">Por favor, no retire su tarjeta</p>
            </div>
          </div>
        )}
        ...
        {status === 'success' && (
          <div className="flex flex-col items-center text-center gap-5 animate-fade-in">
            ...
            <div className="space-y-1">
              <h2 className="text-2xl font-black text-green-400">¡Pago Exitoso!</h2>
              <p className="text-sm text-white/60">Imprimiendo ticket de compra...</p>
            </div>
          </div>
        )}
        ...
        {status === 'error' && (
          <div className="flex flex-col items-center text-center gap-5 animate-fade-in">
            ...
            <div className="space-y-1.5 max-w-xs">
              <h2 className="text-2xl font-black text-red-400">Transacción Declinada</h2>
              <p className="text-sm text-white/60">Intente de nuevo, use otro método de pago o llame a un asistente.</p>
            </div>
          </div>
        )}
