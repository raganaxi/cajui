		...
		<div className={cn("caj-card flex flex-col overflow-hidden", className)}>
			...
			<div
				className={cn(
					...
					status === "open" ? "bg-green-500/10" : "bg-white/[0.05]",
				)}
			>
				<div>
					...
					<p
						className={cn(
							"text-lg font-bold",
							status === "open" ? "text-green-300" : "text-white",
						)}
					>
				...
				<div className="divide-y divide-white/[0.10]">
					...
					{summary.cashInDrawer !== undefined && (
						<SummaryRow
							...
							value={fmt(summary.cashInDrawer)}
						/>
					)}
