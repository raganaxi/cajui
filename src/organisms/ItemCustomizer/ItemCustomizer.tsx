...
export function ItemCustomizer({
	...
	>(() => {
		...
		for (const group of modifiers) {
			if (group.type === "checklist") {
				...
				const defaultIdx = group.options.findIndex(
	...
	return (
		<GlassPanel
			...
			radius="xl"
			className={cn(
				"grid grid-cols-1 md:grid-cols-12 gap-6 w-full max-w-4xl p-6 text-white overflow-hidden",
				className,
			)}
		>
			...
			<div className="md:col-span-4 flex flex-col items-center justify-between bg-white/[0.02] rounded-2xl border border-white/5 p-6 min-h-[380px]">
				<div className="text-center">
					...
					<p className="text-xs text-white/50 mt-1 line-clamp-2">
						{product.description || "Personaliza tu bebida a tu gusto."}
					</p>
				</div>
						...
						<div className="grid grid-cols-4 gap-2">
							{sizes.map((s) => (
								...
								>
									...
									<div className="h-10 flex items-end justify-center mb-2">
										<div
											className={cn(
												...
												selectedSize.id === s.id
													? "border-caj-primary bg-caj-primary/20"
													: "border-white/40 bg-white/5",
											)}
											style={{
												height: `${20 + s.scale * 15}px`,
												width: `${14 + s.scale * 6}px`,
											}}
										/>
									</div>
									...
									{s.extraPrice > 0 && (
										<span className="text-[10px] font-semibold text-green-300 mt-0.5">
											+{fmt(s.extraPrice)}
										</span>
									)}
								</button>
							...
							{group.type === "slider" && (
								<div className="space-y-2">
									<input
										...
										onChange={(e) =>
											handleSliderChange(group.id, parseInt(e.target.value, 10))
										}
										...
										{group.sliderLabels?.map((l, idx) => (
											<button
												...
												className={cn(
													...
													(selections[group.id] as number) === idx &&
														"text-caj-primary font-bold",
												)}
												...
												<div className="flex items-center gap-2">
													...
													<span
														className={cn(
															...
															isChecked
																? "border-caj-primary bg-caj-primary text-white"
				...
				<div className="border-t border-white/10 pt-4 flex flex-col gap-3">
					...
					<div className="flex items-center justify-between gap-4">
						<div className="flex flex-col">
							...
							<span className="text-2xl font-bold text-caj-primary tracking-tight">
								{fmt(totalPrice)}
							</span>
						</div>
