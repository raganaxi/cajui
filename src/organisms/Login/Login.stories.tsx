...
export const DefaultPinAndCredentials: Story = {
	args: {
		...
		onLogin: (creds) =>
			alert(`Intentando ingresar con: ${JSON.stringify(creds)}`),
	},
};
...
export const PinOnly: Story = {
	args: {
		...
		companyBranding: (
			<Branding companyName="Boutique POS" size="sm" themeColor="#ec4899" />
		),
...
export const CredentialsOnly: Story = {
	args: {
		...
		allowCredentials: true,
		companyBranding: (
			<Branding companyName="Corporación ERP" size="sm" themeColor="#06b6d4" />
		),
		onLogin: (creds) =>
			alert(`Credenciales: ${creds.email} / ${creds.password}`),
	},
};
