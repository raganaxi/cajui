...
export function ReceiptPreview({ data, onPrint, compact = false, className }: ReceiptPreviewProps) {
  const {
    folio,
    date,
    time,
      ...
      >
        <div className={cn('p-4', compact ? 'p-3' : 'p-5')}>
            ...
            {items.map((item) => {
              ...
              return (
                <div key={item.id}>
                  ...
                  {item.discount && (
                    <p className="pl-2 text-xs text-gray-500">  Desc {item.discount}%</p>
                  )}
                </div>
              )
            })}
          ...
          <div className="space-y-0.5">
            ...
            {tip > 0 && (
              <ReceiptRow label="Propina:" value={fmt(tip)} />
            )}
          </div>
          ...
          {(paymentMethod || tendered !== undefined) && (
            <>
              <Divider />
              {paymentMethod && (
                <ReceiptRow label="Forma de pago:" value={PAYMENT_LABELS[paymentMethod]} />
              )}
              {tendered !== undefined && (
...
function Divider({ char = '-' }: { char?: string }) {
  return (
    <p className="my-2 overflow-hidden text-gray-400">
      {char.repeat(40)}
    </p>
  )
}
